...
public class ConfigParser {
     ...
     *
     * @param fileName pathname of the configuration file relative to the
     * .minecraft application directory
     ...
     * under Windows 7 shows. For example, the control panel shows "Franklin
     * Gothic" as a font family with two styles "Medium" and "Medium Italic".
     * Yet Java 6 reports both the font <i>family</y> as "Franklin Gothic
     * Medium". These kinds of variations make it difficult to match font names
     * properly, so the approach here it to first look for an exact font face
     * name match, and failing that a substring match. In other words, if the
     * user specified "Times" it would most likely match with "Times New Roman".
     * Even more interesting, is when a font shows up with family name "Latin
     * Wide" in the control panel but Java returns "Wide Latin" for both the
     * font's famil and face name.
     *
     * @param defaultValue the default value to use if the font.name property is
    ...
    public String getFontName(String defaultValue) {
        ...
        for (String LOGICAL_FONTS1 : LOGICAL_FONTS) {
            if (LOGICAL_FONTS1.compareToIgnoreCase(searchName) == 0) {
                return LOGICAL_FONTS1;
            }
        }
     ...
     *
     * @param defaultValue the default value to use if the font.size property is
     * missing or is invalid
