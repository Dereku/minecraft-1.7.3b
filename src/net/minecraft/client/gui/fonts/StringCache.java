...
public class StringCache {
     ...
     * @param colors 32 element array of RGBA colors corresponding to the 16
     ...
     * @param color the initial RGBA color to use when drawing the string;
     * embedded color codes can override the RGB component
     * @param shadowFlag if true, color codes are replaces by a darker version
     ...
     * @param breakAtSpaces set to prefer breaking line at spaces than in the
     * middle of a word
     ...
     * @param shadowFlag ir true, the color code will select a darker version of
     * the color suitable for drop shadows
     ...
     *
     * @param colorList each color change in the string will add a new ColorCode
     * object to this list
     ...
     *
     * @param glyphList will hold all new Glyph objects allocated by
     * layoutFont()
    ...
    private int layoutBidiString(List<Glyph> glyphList, char text[], int start, int limit, ColorCode colors[]) {
        ...
        if (Bidi.requiresBidi(text, start, limit)) {
            ...
            } /* Otherwise text has a mixture of LTR and RLT, and it requires full bidirectional analysis */ else {
            ...
            return advance;
        } /* If text is entirely left-to-right, then insert an EntryText node for the entire string */ else {
     ...
     * @param advance the horizontal advance (i.e. X position) returned by
     * previous call to layoutString()
